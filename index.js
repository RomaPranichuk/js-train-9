...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  if (arr.length === 0) {
    return "Масив не може бути порожнім";
  }
  ...
  if (startIdx >= arr.length) {
    return "Індекс не може бути більшим за довжину масиву";
  }
...
function getPositiveArrayValues(arr) {
  ...
  let positiveArrayValues = values.filter((value) => value > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  if (!Number.isInteger(startIdx) || startIdx < 0) {
    return "Індекс має бути додатним цілим числом";
  }
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let squaredArray = flattenedArray.map((x) => x ** 2);

  let filteredSquaredArray = squaredArray.filter((x) => x > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (!Number.isInteger(n) || n <= 0) {
    return "n має бути додатним цілим числом";
  }

  let lastElements = [...arr].splice(-n);

  let sum = lastElements.reduceRight((acc, cur) => acc + cur);

  lastElements.forEach((el) => console.log(el));
...
function expandArrayByN(arr, n) {
  ...
  let expandedArray = arr.flatMap((x) => Array(n).fill(x));
...
function findDuplicateElements(arr) {
  ...
  let duplicateElements = arr.filter((element, index) => {
